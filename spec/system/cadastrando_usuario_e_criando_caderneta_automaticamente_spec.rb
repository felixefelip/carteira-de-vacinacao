...
describe 'Cadastrando usuário e verificando a caderneta criada automaticamente', type: :feature do
  it 'cadastra o usuário', :aggregate_failures do
    ...
    expect(Vacina.count).to eq(17)
    expect(FabricanteVacina.count).to eq(17)
    expect(DoseDoCalendario.count).to eq(30)
    ...
    recomendacoes_de_vacinas = User.last.pessoa_titular.caderneta.recomendacao_vacinas
    recomendacao_vacina = recomendacoes_de_vacinas.first!

    # save_and_open_page
    expect(recomendacoes_de_vacinas.count).to eq(17)
